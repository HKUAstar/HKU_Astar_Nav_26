...
class dstarlite{
    public:
        ...
        void dstar_main(double real_start_x, double real_start_y);
        ...
        void init_rog_map(double robot_radius, double scan_radius);
        void update_from_esdf(double robot_x, double robot_y, double robot_z);
        ...
        bool arrive_flag = 0;
        // ROG-Map ESDF
        rog_map::ROGMap::Ptr rog_map_ptr_;
        double esdf_robot_radius_ = 0.6;
        double esdf_scan_radius_ = 6.0;
        double current_robot_z_ = 0.0;
};
...
void dstarlite::init_rog_map(double robot_radius, double scan_radius){
    esdf_robot_radius_ = robot_radius;
    esdf_scan_radius_  = scan_radius;
    ros::NodeHandle nh_private("~");  // 私有命名空间，参数路径 /dstarlite/rog_map/...
    rog_map_ptr_ = std::make_shared<rog_map::ROGMap>(nh_private);
    ros::Duration(1.0).sleep();       // 等待第一帧建图完成
    ROS_INFO("ROG-Map initialized, robot_radius=%.2f scan_radius=%.2f", robot_radius, scan_radius);
}

void dstarlite::update_from_esdf(double robot_x, double robot_y, double robot_z){
    if(!rog_map_ptr_) return;
    current_robot_z_ = robot_z;
    for_dynamic_map_round++;

    int cx = from_real_x_to_map_x(robot_x);
    int cy = from_real_y_to_map_y(robot_y);
    int radius_cells = (int)(esdf_scan_radius_ / resolution) + 1;

    for(int ddx = -radius_cells; ddx <= radius_cells; ddx++){
        for(int ddy = -radius_cells; ddy <= radius_cells; ddy++){
            int nx = cx + ddx, ny = cy + ddy;
            if(nx < 0 || nx >= max_x || ny < 0 || ny >= max_y) continue;

            double wx = nx * resolution + initial_x;
            double wy = ny * resolution + initial_y;
            double dist = rog_map_ptr_->getDistance(rog_map::Vec3f(wx, wy, robot_z));

            // 只处理真正危险的格子，滑动窗口视角变化不触发重规划
            if(dist >= esdf_robot_radius_) continue;

            // 距离层死区，防止噪声抖动
            if(std::abs(dist - map[nx][ny]->last_esdf_dist) < 0.05) continue;
            map[nx][ny]->last_esdf_dist = dist;

            double new_occ = (dist <= 0.0) ? 100.0 : 100.0 * (1.0 - dist / esdf_robot_radius_);
            double clamped = std::max(new_occ, map[nx][ny]->static_obstacle_possibility);

            // 更新衰减跟踪集合
            changed_obstacle_nodes.erase(map[nx][ny]);
            map[nx][ny]->round_for_dynamic_map = for_dynamic_map_round;
            changed_obstacle_nodes.insert(map[nx][ny]);

            if(std::abs(clamped - map[nx][ny]->obstacle_possibility) > 5.0){
                map[nx][ny]->obstacle_possibility = clamped;
                dstar_update_node(map[nx][ny]);
            }
        }
    }

    // 衰减：长时间未重新观测的动态障碍格子，恢复为静态值
    while(!changed_obstacle_nodes.empty() && ros::ok()){
        ...
        if(cur->round_for_dynamic_map + 60 <= for_dynamic_map_round){
            ...
            cur->last_esdf_dist = -2.0;
            if(std::abs(cur->obstacle_possibility - cur->static_obstacle_possibility) > 5.0){
...
void dstarlite::dstar_main(double real_start_x, double real_start_y){
...
std::atomic<bool> get_goal_info{false};  // AsyncSpinner 并发写，不能用普通 bool
...
int main(int argc, char **argv){
    ...
    double esdf_robot_radius, esdf_scan_radius;
    if (!nh.getParam(node_name+"/esdf_robot_radius", esdf_robot_radius)) esdf_robot_radius = 0.6;
    if (!nh.getParam(node_name+"/esdf_scan_radius",  esdf_scan_radius))  esdf_scan_radius  = 6.0;
    ros::AsyncSpinner spinner(2);  // 2线程：一个处理ROG-Map回调，一个处理goal
    spinner.start();

    dstar.init_rog_map(esdf_robot_radius, esdf_scan_radius);
    ...
    while(ros::ok()){
        ...
        if(have_first_goal){
            ...
            if(abs(real_goal_x - transformStamped.transform.translation.x) < 0.25 && abs(real_goal_y - transformStamped.transform.translation.y) < 0.25){
                ...
                if(++esdf_frame_count % 10 == 0){
                    dstar.update_from_esdf(
