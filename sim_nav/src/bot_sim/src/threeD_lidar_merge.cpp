...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
        ...
        for (int i = 0; i < scan_record_left.points.size(); i++){
            ...
            int idx = scan_new.points.size() - 1;
            
            scan_new.points[idx].x += 0.02843;
            double y_temp = scan_record_left.points[i].y + 0.14337;
            double z_temp = scan_record_left.points[i].z;
            scan_new.points[idx].y = y_temp * cos_a - z_temp * (-sin_a);
            scan_new.points[idx].z = y_temp * (-sin_a) + z_temp * cos_a;
        }
