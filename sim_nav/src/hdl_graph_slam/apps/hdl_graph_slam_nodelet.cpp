...
private:
  ...
  void cloud_callback(const nav_msgs::OdometryConstPtr& odom_msg, const sensor_msgs::PointCloud2::ConstPtr& cloud_msg) {
    ...
    publish_odom2map_transform(stamp);
  ...
  void publish_odom2map_transform(const ros::Time& stamp) {
    if(!odom2map_pub.getNumSubscribers()) {
      return;
    }

    std::lock_guard<std::mutex> lock(odom2map_publish_mutex);
    if(!last_odom2map_stamp.isZero() && stamp <= last_odom2map_stamp) {
      return;
    }

    trans_odom2map_mutex.lock();
    Eigen::Matrix4f odom2map = trans_odom2map;
    trans_odom2map_mutex.unlock();

    geometry_msgs::TransformStamped ts = matrix2transform(stamp, odom2map, map_frame_id, odom_frame_id);
    odom2map_pub.publish(ts);
    last_odom2map_stamp = stamp;
  }
  ...
  void optimization_timer_callback(const ros::WallTimerEvent& event) {
    ...
    publish_odom2map_transform(keyframe->stamp);
...
private:
  ...
  std::mutex odom2map_publish_mutex;
  Eigen::Matrix4f trans_odom2map;
  ros::Time last_odom2map_stamp;
