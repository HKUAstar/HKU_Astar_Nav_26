...
public:
  ...
  virtual void onInit() {
    ...
    points_sub = nh.subscribe("/velodyne_points", points_sub_queue_size, &PrefilteringNodelet::cloud_callback, this);
    points_pub = nh.advertise<sensor_msgs::PointCloud2>("/filtered_points", points_pub_queue_size);
    colored_pub = nh.advertise<sensor_msgs::PointCloud2>("/colored_points", points_pub_queue_size);
  }
...
private:
  void initialize_params() {
    ...
    max_input_delay = private_nh.param<double>("max_input_delay", 0.0);
    max_num_points = std::max(0, private_nh.param<int>("max_num_points", 0));
    points_sub_queue_size = std::max(1, private_nh.param<int>("points_sub_queue_size", 16));
    points_pub_queue_size = std::max(1, private_nh.param<int>("points_pub_queue_size", 8));

    base_link_frame = private_nh.param<std::string>("base_link_frame", "");
    transform_timeout = private_nh.param<double>("transform_timeout", 0.2);
  }
  ...
  void cloud_callback(const pcl::PointCloud<PointT>& src_cloud_r) {
    ...
    if(max_input_delay > 0.0) {
      const ros::Time stamp = pcl_conversions::fromPCL(src_cloud->header.stamp);
      const ros::Duration lag = ros::Time::now() - stamp;
      if(lag.toSec() > max_input_delay) {
        NODELET_WARN_STREAM_THROTTLE(1.0, "dropping stale input cloud delayed by " << lag.toSec() << " s");
        return;
      }
    }
    ...
    if(!base_link_frame.empty() && base_link_frame != src_cloud->header.frame_id) {
      tf::StampedTransform transform;
      try {
        if(!tf_listener.waitForTransform(base_link_frame, src_cloud->header.frame_id, ros::Time(0), ros::Duration(transform_timeout))) {
          NODELET_WARN_STREAM_THROTTLE(5.0, "skipping cloud because transform is unavailable: " << src_cloud->header.frame_id << " -> " << base_link_frame);
          return;
        }

        tf_listener.lookupTransform(base_link_frame, src_cloud->header.frame_id, ros::Time(0), transform);
      } catch(const tf::TransformException& ex) {
        NODELET_WARN_STREAM_THROTTLE(5.0, "skipping cloud because transform lookup failed: " << src_cloud->header.frame_id << " -> " << base_link_frame << " (" << ex.what() << ")");
        return;
      }
    ...
    filtered = limit_num_points(filtered);
  ...
  pcl::PointCloud<PointT>::ConstPtr limit_num_points(const pcl::PointCloud<PointT>::ConstPtr& cloud) const {
    if(max_num_points <= 0 || cloud->size() <= static_cast<std::size_t>(max_num_points)) {
      return cloud;
    }

    NODELET_WARN_STREAM_THROTTLE(5.0, "limiting filtered cloud from " << cloud->size() << " to " << max_num_points << " points");

    pcl::PointCloud<PointT>::Ptr limited(new pcl::PointCloud<PointT>());
    limited->reserve(max_num_points);

    for(int i = 0; i < max_num_points; ++i) {
      const std::size_t index = static_cast<std::size_t>(i) * cloud->size() / static_cast<std::size_t>(max_num_points);
...
private:
  ...
  double max_input_delay;
  int max_num_points;
  int points_sub_queue_size;
  int points_pub_queue_size;
